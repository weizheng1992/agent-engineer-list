...
const server = new Server(
  { name: "mini-cursor-fs-server", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "read_file") {
    ...
    // Resolve safety checks (can restrict to workspace root if desired, or let it read absolute)
...
console.error("Local MCP Server connected via stdio transport!");
